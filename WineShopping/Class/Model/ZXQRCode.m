...
@interface ZXQRCode ()<AVCaptureMetadataOutputObjectsDelegate>
...
+ (instancetype)shareInstance{
    ...
    static ZXQRCode *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[ZXQRCode alloc]init];
    });
    return instance;
}
...
+ (UIImage *)buildQRCodeWithMessage:(NSString *)message
    ...
    ZXQRCode *instance =[ZXQRCode shareInstance];
    instance.QRfilter =[CIFilter filterWithName:( type ==ZXCodeTypeQRCode ? @"CIQRCodeGenerator" :@"CICode128BarcodeGenerator")];
    //恢复默认
    [instance.QRfilter setDefaults];
    NSData *data;
    if (type == ZXCodeTypeQRCode) {
        //纠错水平  L M Q H
