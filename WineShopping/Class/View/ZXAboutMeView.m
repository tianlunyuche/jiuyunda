...
- (void)initView
{
    ...
    bgImageView.image           = [ZXTool imageWithView:[[UIApplication sharedApplication].delegate window] withBlurRadiu:5];
    [self addSubview:bgImageView];
    
    UIView *bgView              = [[UIView alloc]initWithFrame:self.bgFrame];
    bgView.layer.borderWidth    = 0.7;
    bgView.layer.borderColor    = kRGBAColor(180, 180, 180, 1).CGColor;
    ...
    [self.titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        btn.titleLabel.font     = [UIFont zxNormalFont:16];
        ...
        [btn setTitleColor:kRGBAColor(70, 70, 70, 1) forState:UIControlStateNormal];
