...
@implementation ZXGoodManagerView
- (instancetype)init
{
    self = [super init];
    if (self)
...
- (void)configView
{
    ...
    _wtkNumLable.font               = [UIFont zxNormalFont:16];
    _wtkNumLable.text               = @"";
...
- (void)leftBtnClick:(UIButton *)btn
{
    ...
    if (self.goods.num == 0)
    {
        [[ZXShoppingManager manager].goodsDic removeObjectForKey:self.goods.id];
...
- (void)rightBtnClick:(UIButton *)btn
{
    ...
    [[ZXShoppingManager manager].goodsDic setObject:self.goods forKey:self.goods.id];
    ZXLog(@"self.goods.id =%@",self.goods.id);
    //    角标
    [ZXUser currentUser].bageValue++;
