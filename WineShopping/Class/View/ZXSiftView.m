...
@interface ZXSiftView ()<UITableViewDelegate,UITableViewDataSource>
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        self.backgroundColor        = kRGBAColor(70, 70, 70, 0.8);
...
- (void)configView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"ZXSiftTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"cell"];
...
{
    ZXSiftModel *model             = self.dataArray[indexPath.section][@"data"][indexPath.row];
    model.isSelected                = !model.isSelected;
    ZXSiftTableViewCell *cell      = [tableView cellForRowAtIndexPath:indexPath];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    titleLabel.font                 = [UIFont zxNormalFont:16];
    titleLabel.textColor            = kRGBAColor(80, 80, 80, 1);
    ...
    subTitle.font                   = [UIFont zxNormalFont:13];
    subTitle.textColor              = kRGBAColor(100, 100, 100, 1);
    ...
    line.backgroundColor            = kRGBAColor( 150, 150, 150, 1);
...
{
    ZXSiftTableViewCell *cell      = [tableView dequeueReusableCellWithIdentifier:@"cell"];
...
- (UIView *)headerView
{
    if (!_headerView)
    {
        ...
        cancelBtn.titleLabel.font       = [UIFont zxNormalFont:16];
        [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        [cancelBtn setTitleColor:kRGBAColor(80, 80, 80, 1) forState:UIControlStateNormal];
        cancelBtn.tag                   = 1;
        ...
        titleLabel.text                 = @"筛选";
        titleLabel.textColor            = kRGBAColor(30, 30, 30, 1);
        titleLabel.font                 = [UIFont zxNormalFont:20];
        [self.headerView addSubview:titleLabel];
        
        UIButton *confirmBtn            = [UIButton buttonWithType:UIButtonTypeCustom];
        confirmBtn.frame                = CGRectMake(self.tableView.frame.size.width - 64, 20, 40, 30);
        confirmBtn.titleLabel.font      = [UIFont zxNormalFont:16];
        [confirmBtn setTitle:@"确定" forState:UIControlStateNormal];
        [confirmBtn setTitleColor:kRGBAColor(80, 80, 80, 1) forState:UIControlStateNormal];
        confirmBtn.tag                  = 2;
...
- (UIView *)footView
{
    if (!_footView)
    {
        ...
        btn.backgroundColor             = kRGBAColor(230, 230, 230, 1);
        btn.titleLabel.font             = [UIFont zxNormalFont:19];
        [btn setTitleColor:kRGBAColor(110, 110, 110, 1) forState:UIControlStateNormal];
