...
@interface ZXGoodsVC ()<WKNavigationDelegate,WKUIDelegate,UIScrollViewDelegate>

@property(nonatomic,strong,readwrite)ZXGoodsViewModel  *viewModel;
...
///评论
@property(nonatomic,strong)ZXCommentTableView          *tableView;
///评论head
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:kRGBAColor(80, 80, 80, 1)};
...
- (void)initView
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/userinfos/%@/products/%@/desc",IMG_URL,[ZXUser currentUser].bid,self.viewModel.goods.id];
...
- (void)headBtnClick:(UIButton *)btn
{
    ...
    ZXCommentBtn *lastBtn = self.headBtnArray[self.lastSelect];
    lastBtn.w_titleColor = kRGBAColor(80, 80, 80, 1);
    ZXCommentBtn *newBtn = (ZXCommentBtn *)btn;
...
{
    if ([ZXUser currentUser].bageValue >= 0)
    {
        self.bageLabel.hidden               = NO;
    }
    [self.bageLabel setTitle:[NSString stringWithFormat:@"%ld",[ZXUser currentUser].bageValue] forState:UIControlStateNormal];
    @weakify(self);
    [UIView animateWithDuration:0.2 animations:^{
        @strongify(self);
        self.bageLabel.frame                = CGRectMake(_shoppingCarBtn.frame.size.width / 2.0 + 10, 2, 30, 30);
        
        self.bageLabel.titleLabel.font      = [UIFont zxNormalFont:17];
        
    } completion:^(BOOL finished) {
        @strongify(self);
        [UIView animateWithDuration:0.2 animations:^{
            self.bageLabel.frame                = CGRectMake(_shoppingCarBtn.frame.size.width / 2.0 + 10, 2, 25, 25);
            self.bageLabel.titleLabel.font      = [UIFont zxNormalFont:14];
...
- (UIButton *)shoppingCarBtn
{
    if (!_shoppingCarBtn)
    {
        ...
        _shoppingCarBtn.backgroundColor     = kRGBAColor(30, 30, 30, 0.8);
...
- (UISegmentedControl *)titleView
{
    if (!_titleView)
    {
        ...
        NSDictionary *selectDic = @{NSForegroundColorAttributeName :kRGBAColor(70, 70, 70, 1),NSFontAttributeName:[UIFont zxNormalFont:14]};
        NSDictionary *normalDic = @{NSForegroundColorAttributeName :kRGBAColor(140, 140, 140, 1),NSFontAttributeName:[UIFont zxNormalFont:14]};
...
- (UIView *)headView
{
    if (!_headView)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ZXCommentBtn *btn  = [ZXCommentBtn buttonWithTitle:title[i] subTitle:@"0"];
            btn.frame           = CGRectMake(width * i, 0, width, 50);
            btn.tag             = i;
            btn.w_titleColor    = i == 0 ? THEME_COLOR : kRGBAColor(80, 80, 80, 1);
...
- (ZXCommentTableView *)tableView
{
    if (!_tableView)
    {
        _tableView = [[ZXCommentTableView alloc] initWithFrame:CGRectMake(kWidth, 50, kWidth, kHeight - 50 - 64) style:UITableViewStylePlain];
    }
    return _tableView;
}
