...
@interface ZXMeVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property(nonatomic,strong)ZXMeHeaderView      *headView;
/**设置按钮*/
...
- (void)initView
{
    ...
    _titleArray = @[@"推荐有奖",@"意见反馈",@"客服热线",@"酒运达Web介绍"];
    self.tableView.dataSource       = self;
    self.tableView.delegate         = self;
    self.tableView.separatorStyle   = UITableViewCellSeparatorStyleNone;
    [self.tableView registerNib:[UINib nibWithNibName:@"ZXMeTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"cell"];
...
- (void)updateNavi:(id )x
{
    ...
    if (y >= 0)
    {
        ...
        ZXLog(@"%f",a);
        self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:kRGBAColor(70, 70, 70, a)};
        if (a < 0.9 && a >= 0)
        {
            ZXLog(@"11111");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 2)
    {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"客服热线" message:@"全国客服热线：10010" preferredStyle:UIAlertControllerStyleActionSheet];
    ...
    else
    {
        ...
        if (!footView)
        {
            ...
            [titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ZXMeBtn *btn = [ZXMeBtn buttonWithTitle:obj imageName:imgArray[idx]];
                btn.tag     = idx;
...
{
    ZXMeTableViewCell *cell        = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    ...
    else
    {
        ...
        if (indexPath.row == 2)
        {
            [cell updateTitle:self.titleArray[indexPath.row] subTitle:@"QQ240394381"];
        }
    }
...
- (UIView *)headView
{
    if (!_headView)
    {
        _headView = [[ZXMeHeaderView alloc]initWithFrame:CGRectMake(0, 0, kWidth, 200)];
