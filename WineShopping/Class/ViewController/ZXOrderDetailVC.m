...
@property(nonatomic,strong)ZXOrderDetailViewModel  *viewModel;

@property(nonatomic,strong)UITableView              *tableView;

@property(nonatomic,strong)UISegmentedControl       *segment;

@property(nonatomic,strong)ZXOrderStateTableView   *statusTable;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZXOrderDetailTableViewCell *cell = nil;
    if (indexPath.section == 0)
    {
        cell = [tableView dequeueReusableCellWithIdentifier:cell0];
        if (!cell)
        {
            NSArray *array = [[NSBundle mainBundle]loadNibNamed:@"ZXOrderDetailTableViewCell" owner:nil options:nil];
            cell = array[0];
        }
...
- (ZXOrderStateTableView *)statusTable
{
    if (!_statusTable)
    {
        _statusTable = [[ZXOrderStateTableView alloc]initWithFrame:CGRectMake(0, 64, kWidth, kHeight) style:UITableViewStylePlain];
...
- (UIView *)bottomView
{
    if (!_bottomView)
    {
        ...
        _bottomView.backgroundColor = kRGBAColor(30, 30, 30, 0.8);
...
- (UIButton *)doneBtn
{
    if (!_doneBtn)
    {
        ...
        _doneBtn.titleLabel.font = [UIFont zxNormalFont:19];
