...
@property(nonatomic,strong)ZXOrderTableView    *tableView;

@property(nonatomic,strong)UIButton             *rightBtn;

@property(nonatomic,strong)ZXOrderMenuView     *menuView;
...
- (ZXOrderTableView *)tableView
{
    if (!_tableView)
    {
        _tableView = [[ZXOrderTableView alloc]initWithFrame:CGRectMake(0, 0, kWidth, kHeight) style:UITableViewStylePlain];
    }
    return _tableView;
}
- (UIButton *)rightBtn
...
- (ZXOrderMenuView *)menuView
{
    if (!_menuView)
    {
        _menuView = [[ZXOrderMenuView alloc]initWithFrame:CGRectMake(0, 64, kWidth, kHeight) withData:@[@"全部订单",@"待付款",@"配送中",@"已配送",@"已完成"]];
