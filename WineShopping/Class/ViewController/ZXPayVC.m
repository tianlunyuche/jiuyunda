...
@interface ZXPayVC ()<UITableViewDelegate,UITableViewDataSource>

@property(nonatomic,strong)ZXPayViewModel  *viewModel;
...
- (void)initView
{
    ...
    self.priceLabel.font            = [UIFont zxNormalFont:16];
    self.priceLabel.textColor       = [UIColor whiteColor];
    self.priceLabel.text            = [NSString stringWithFormat:@"实付款 ：¥%.2f",self.viewModel.price];
    
    
    self.payBtn.titleLabel.font     = [UIFont zxNormalFont:18];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        if (!cell)
        {
            ...
            cell.textLabel.textColor        = kRGBAColor(150, 150, 150, 1);
            UIView *line                    = [[UIView alloc]initWithFrame:CGRectMake(12, 43.6, kWidth - 12, 0.4)];
            line.backgroundColor            = kRGBAColor(220, 220, 220, 1);
    ...
    else if (indexPath.section == 1)
    {
        ...
        {
            ZXOrderDetailTableViewCell *cell   = [tableView dequeueReusableCellWithIdentifier:@"CELL2"];
            if (!cell)
            {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    header.backgroundColor = kRGBAColor(240, 240, 240, 1);
    return header;
}
