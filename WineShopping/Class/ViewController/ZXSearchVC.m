...
#import "ZXSearchResultView.h"

#define HISTORY @"historySearch"

@interface ZXSearchVC ()<UISearchBarDelegate>
...
///搜索结果
@property(nonatomic,strong)ZXSearchResultView      *resultView;
@end
...
- (void)bindViewModel
{
    ...
    [self.viewModel.searchCommand.executionSignals.switchToLatest subscribeNext:^(id x) {
        ...
        NSArray *array = [ZXGood mj_objectArrayWithKeyValuesArray:x context:nil];
...
- (void)resetNavi
{
    ...
    _searchBar.layer.borderColor    = kRGBAColor(200, 200, 200, 1).CGColor;
...
- (void)initView{
    self.view.backgroundColor       = kRGBAColor(253, 253, 253, 1);
    [self.view addSubview:self.bgView];
    UILabel *hotLabel               = [[UILabel alloc]initWithFrame:CGRectMake(0, 64 * ZOOM_SCALL, kWidth, 46 * ZOOM_SCALL)];
    hotLabel.textColor              = kRGBAColor(150, 150, 150, 1);
    hotLabel.font                   = [UIFont zxNormalFont:16];
    hotLabel.text                   = @"  热门商品";
    ...
    historyLabel.textColor          = kRGBAColor(150, 150, 150, 1);
    historyLabel.font               = [UIFont zxNormalFont:16];
    ...
    _deleteBtn.titleLabel.font      = [UIFont zxNormalFont:17];
    _deleteBtn.backgroundColor      = kRGBAColor(199, 199, 199, 1);
...
- (ZXSearchResultView *)resultView
{
    if (!_resultView)
    {
        _resultView = [[ZXSearchResultView alloc]initWithFrame:CGRectMake(0, 64, kWidth, kHeight - 64)];
