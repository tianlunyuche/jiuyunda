...
+ (void)beginAddAnimationWithImageView:(UIImageView *)imageView
    ...
    UIWindow *window =[kApplication.delegate window];
    ...
    layer.contents      =imageView.layer;
...
{
    LAContext *zxContext =[[LAContext alloc] init];
    NSError *autoError =nil;
    NSString *title =@"开启指纹验证";
    //判断设备是否支持
    if([zxContext canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:&autoError]){
        
        [zxContext evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics localizedReason:title reply:^(BOOL success, NSError * _Nullable error) {
            if (success) {
                [ZXUser currentUser].isTouchID =YES;
                block(@"验证成功");
            }else{
                ZXLog(@"%ld",error.code);
                // 错误码 error.code
...
+ (BOOL)delegateTouchID{
    [ZXUser currentUser].isTouchID =NO;
    return YES;
}
...
+(RACSignal *)shared
{
    ...
    //    blurView.tintColor      = kRGBACOLOR(235, 235, 235, 0.5);
    UIWindow *window =[kApplication.delegate window];
    ...
    [btnImages enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        NSInteger row =idx /3;
        NSInteger col =idx %3;
        
        ZXShareBtn *btn =[ZXShareBtn button];
        ...
        [[btn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            ZXLog(@"UIControlEventTouchUpInside");
